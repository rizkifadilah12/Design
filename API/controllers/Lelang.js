...
export const updateLelang = async(req, res) =>{
  const lelang = await Lelang.findOne({
    attributes: [
      `id`, `uuid`, `barangId`, `tgl_mulai`, `tgl_akhir`, `harga_awal`, `status`, `created_by`, `created_date`, `masyarakatId`, `harga_akhir`, `confirm_date`, `createdAt`, `updatedAt`
    ],
    where:{
        id: req.params.id
    }
...
export const getMaxPenawaran = async(req, res) =>{
  const selectViewSql = ` 
  SELECT *
  FROM get_max_pemawaran where id_lelang = ${req.params.id} order by asc limit = 1
          `;
// Execute the SQL query to select data from the view
db
.query(selectViewSql, { type: sequelize.QueryTypes.SELECT })
